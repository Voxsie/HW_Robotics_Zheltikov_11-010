...
img = cv2.imread('../Homework_7/source/hw7picture.jpeg')
scale_up_x = 0.5
scale_up_y = 0.5
scale_down = 0.6
...
for i in range(0, rows):
    for j in range(0, columns):
        ...
        cv2.imwrite('../Homework_7/source/v('+ str(i) +':'+ str(j) + ')' + '.png', patch)
...
for i in range(0, rows):
    for j in range(0, columns):
        img = plt.imread('../Homework_7/source/v('+ str(i) +':'+ str(j) + ')' + '.png')
        fig2[i,j].axis('off')
        fig2[i,j].imshow(img)
plt.tight_layout()
plt.show()